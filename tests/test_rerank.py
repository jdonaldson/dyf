...
class TestRerankMmr:
    def test_returns_top_k(self, synthetic_retrieval):
        s = synthetic_retrieval
        result = rerank_mmr(s["query_emb"], s["candidate_indices"], s["emb_normed"], top_k=5, lam=0.5)
        assert len(result) == 5

    def test_more_diverse_than_standard(self, synthetic_retrieval):
        s = synthetic_retrieval
        standard = rerank_standard(s["sims"], s["candidate_indices"], top_k=8)
        mmr = rerank_mmr(s["query_emb"], s["candidate_indices"], s["emb_normed"], top_k=8, lam=0.5)
    ...
    def test_lam_1_equals_standard(self, synthetic_retrieval):
        ...
        mmr = rerank_mmr(s["query_emb"], s["candidate_indices"], s["emb_normed"], top_k=5, lam=1.0)
...
class TestRerankBridgeBoost:
    def test_returns_top_k(self, synthetic_retrieval):
        s = synthetic_retrieval
        result = rerank_bridge_boost(s["sims"], s["candidate_indices"], s["bridge_scores"], top_k=5, alpha=0.3)
        assert len(result) == 5

    def test_bridges_get_promoted(self, synthetic_retrieval):
        s = synthetic_retrieval
        standard = rerank_standard(s["sims"], s["candidate_indices"], top_k=10)
        boosted = rerank_bridge_boost(s["sims"], s["candidate_indices"], s["bridge_scores"], top_k=10, alpha=0.5)
    ...
    def test_alpha_zero_equals_standard(self, synthetic_retrieval):
        s = synthetic_retrieval
        standard = rerank_standard(s["sims"], s["candidate_indices"], top_k=5)
        boosted = rerank_bridge_boost(s["sims"], s["candidate_indices"], s["bridge_scores"], top_k=5, alpha=0.0)
...
class TestRerankBridgeMmr:
    ...
    def test_meta_clusters_suppressed(self, synthetic_retrieval):
        ...
        meta_in_no = sum(1 for i in result_no_meta if s["cluster_labels"][i] == 1)
        meta_in_yes = sum(1 for i in result_with_meta if s["cluster_labels"][i] == 1)
