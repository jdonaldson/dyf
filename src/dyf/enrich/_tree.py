...
def _collect_descendant_indices(node_id, children_of, leaf_batches):
    ...
    return np.concatenate([_collect_descendant_indices(k, children_of, leaf_batches) for k in kids])


def _label_branch(node, children_of, by_id, leaf_batches, titles, config: _LabelingConfig):
    ...
    for kid_id in kids_sorted:
        ...
        if kn["num_items"] < config.min_child_size:
            continue
        all_idx = _collect_descendant_indices(kid_id, children_of, leaf_batches)
        if len(all_idx) < 3:
            continue
...
def label_tree_bottomup(
    ...
    for i, node in enumerate(target_nodes):
        nid, b_label, k_labels = _label_branch(node, children_of, by_id, leaf_batches, titles, config)
        ...
        n_kids = len(k_labels)
        logger.info(
            f"    [{i + 1}/{len(target_nodes)}] {b_label:<35s} ({node['num_items']:>6,} items, {n_kids} children)"
        )
        for kid_id, kid_label in sorted(k_labels.items(), key=lambda x: -by_id[x[0]]["num_items"]):
            kn = by_id[kid_id]
...
def enrich_tree(dyf_path, model="gpt-oss:20b", target_depth=3, samples_per_child=8, output_path=None):
