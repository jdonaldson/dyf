...
def suggest_n_neighbors(embeddings: np.ndarray, num_bits: int = 12, min_k: int = 15, max_k: int = 100) -> int:
    ...
    logger.info(f"  DYF: {n_buckets} buckets, mean_size={mean_size:.0f}, suggested n_neighbors={suggested}")
    return suggested
...
def orient_landscape(coords: np.ndarray) -> np.ndarray:
    ...
    yr = np.ptp(out[:, 1])
    logger.info(
        f"    Landscape orient: rotated {np.degrees(theta):.1f}°, spread X={xr:.2f} Y={yr:.2f} (ratio {xr / yr:.2f})"
    )
    return out
...
def enrich_project(
    ...
    if fisher_col:
        ...
        if fisher_parquet:
            ...
            if fisher_col in df.columns:
                ...
                logger.warning(f"  column '{fisher_col}' not in {fisher_parquet}, skipping Fisher weighting")
                raw_vals = None
        elif fisher_col in data.get("fields", {}):
            raw_vals = data["fields"][fisher_col]
        else:
            logger.warning(f"  fisher_col='{fisher_col}' not found, skipping Fisher weighting")
            raw_vals = None
        ...
        if raw_vals is not None:
            ...
            logger.info(
                f"  Fisher weighting applied ({fisher_col}): top-5 dims {np.argsort(fisher_weights)[-5:][::-1]}"
            )
        ...
        if diag_path.exists():
            ...
            if label_cols:
                ...
                for d in diags:
                    ...
                    logger.info(f"    {d.name}: lift={d.lift:.1f}x  purity={d.knn_purity:.3f}{flag}")
                under = [d for d in diags if d.lift < 3.0]
                if under:
                    logger.warning(
                        f"  {len(under)} axis(es) under-served. "
    ...
    coords = run_umap(embeddings, n_neighbors=dyf_k, n_components=n_components, densmap=densmap)
    ...
    new_sf = {
        ...
        "umap_z": (coords[:, 2].astype(np.float32) if n_components >= 3 else np.zeros(len(coords), dtype=np.float32)),
    }
    new_meta = {
    ...
    new_meta["_provenance_level_1"] = json.dumps(
        provenance_to_dict(
            create_provenance(
                artifact_type="dyf",
                n_items=len(embeddings),
                source_paths=[str(dyf_path)],
                params={"n_components": n_components, "densmap": densmap, "fisher_col": fisher_col},
            )
        )
    )
    ...
    rewrite_lazy_index(dyf_path, new_stored_fields=new_sf, new_metadata=new_meta, output_path=out)
